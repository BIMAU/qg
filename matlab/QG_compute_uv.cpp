...
{
    if (nrhs != 2)
        mexErrMsgIdAndTxt("QG_rhs:nrhs", "Two input arguments required.");
    if (nlhs != 2)
        mexErrMsgIdAndTxt("QG_rhs:nlhs", "Two output arguments required.");

    // Get back the QG object
    if (mxGetNumberOfElements(prhs[0]) != 1 || mxGetClassID(prhs[0]) != mxUINT64_CLASS)
        mexErrMsgIdAndTxt("QG_rhs:prhs", "Input must be a real uint64 scalar.");
