...
{
    if (nrhs != 2)
        mexErrMsgIdAndTxt("QG_solve:nrhs", "Two input arguments required.");
    if (nlhs != 1)
        mexErrMsgIdAndTxt("QG_solve:nlhs", "One output argument required.");

    // Get back the QG object
    if (mxGetNumberOfElements(prhs[0]) != 1 || mxGetClassID(prhs[0]) != mxUINT64_CLASS)
        mexErrMsgIdAndTxt("QG_solve:prhs", "Input must be a real uint64 scalar.");
